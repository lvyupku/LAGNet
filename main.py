...
def parse_option():
    ...
    #datapath
    parser.add_argument('--adj_path', type=str, help='adjfile path')
    parser.add_argument('--vec_path', type=str, help='word vector file path')
    parser.add_argument('--dataset', type=str, help='dataset type')
    parser.add_argument("--trainset", type=str, help='path to train dataset')
    parser.add_argument("--validset", type=str, help='path to validation dataset')
    parser.add_argument("--testset", type=str, help='path to test dataset')
    parser.add_argument("--train_csv_path", type=str, help='path to train csv file')
    parser.add_argument("--valid_csv_path", type=str, help='path to validation csv file')
...
def main(config):
    ...
    criterion = AsymmetricLossMultiLabel(gamma_pos=a)
...
#                 loss += criterion(outputs[i], targets[i])
    ...
    metr = compute_metrics(y_pred, y_true)
    print('mertic', metr)
...
def validate(config, data_loader, model, is_validation):
    ...
    for idx, batch in enumerate(data_loader):
        ...
        y_pred.append(torch.sigmoid(output).cpu().detach().numpy())
    ...
    auc = multiAuc(y_true=y_true, y_pred=y_pred)
    plotRoc(y_true, y_pred, is_validation)
    print('AUC:', auc) 
    print('metric', metr)        
    logger.info(f'{valid_or_test} MEAN AUC: {val_auc_mean:.5f}')

    return val_auc_mean, loss_meter.avg
